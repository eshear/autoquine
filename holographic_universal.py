#!/usr/bin/env python3
"""
(2, 8) Holographic TM — Universal Minsky Search

Finds a TM that handles VARIABLE-LENGTH payloads, testing with
the full A=2 Minsky cycle (payloads of length 2, 3, and 4).

If the TM handles A=2 correctly, it should generalize to arbitrary A.
"""
...
def add_chain_step(solver, Q, S, D, sid, payload_in, payload_out,
                   spc, x_max, left_pad):
    """
    Add one step: TM starts with payload_in at left_pad, must produce
    payload_out at left_pad after spc steps, with boundaries restored.
    """
    ...
    # Cleanup: boundaries preserved
    for x in range(left_pad):
        solver.add(T[spc][x] == 0)
    for x in range(left_pad + len(pay_out), x_max):
...
def search(max_sing=4, spc=12, x_max=18, left_pad=6, timeout_s=600):
    """
    Search for TM handling the full A=2 Minsky cycle.
    """
    # Full A=2 cycle (both instructions + zero tests)
    steps = [
        # Instruction 0: dec A, inc B
        ([1, 1, 3],    [1, 3, 4],    "I0: A=2→1, B=0→1"),
        ([1, 3, 4],    [3, 4, 4],    "I0: A=1→0, B=1→2"),
        # Zero test: switch to instr 1
        ([3, 4, 4],    [5, 4, 4],    "I0→I1: A=0 switch"),
        # Instruction 1: dec B, inc A
        ([5, 4, 4],    [1, 5, 4],    "I1: B=2→1, A=0→1"),
        ([1, 5, 4],    [1, 1, 5],    "I1: B=1→0, A=1→2"),
        # Zero test: switch to instr 0
        ([1, 1, 5],    [1, 1, 3],    "I1→I0: B=0 switch"),
    ]

    print("=" * 60)
    print(f"(2,8) Holographic TM — Universal Minsky Search")
    print(f"spc={spc}, max_sing={max_sing}, tape={x_max}, pad={left_pad}")
    print(f"Steps in A=2 cycle:")
    ...
    if result == sat:
        ...
        if all_ok:
            ...
            # Now test generalization: A=3, A=4
            for a_val in [1, 2, 3, 4, 5]:
                x_big = 50
                lp = 20
                payload = [1]*a_val + [3]
                tape_big = [0]*lp + payload + [7]*(x_big-lp-len(payload))
                hist = sim(table, tape_big, lp, 0, spc * (2*a_val + 2) * 2, x_big)

                print(f"\n--- A={a_val}, B=0 ({spc*(2*a_val+2)*2} steps) ---")
                for t in range(0, min(len(hist), spc*(2*a_val+2)*2+1), spc):
                    tp, hd, st = hist[t]
                    l = 0
                    ...
                    pay = tp[l:r+1]
                    ds = '>' if st == 0 else '<'
                    a_ct = sum(1 for s in pay if s == 1)
                    b_ct = sum(1 for s in pay if s == 4)
                    mk = 'I0' if 3 in pay else ('I1' if 5 in pay else '??')
                    match = ''
                    if hd == lp and st == 0:
                        if sorted(pay) == sorted([1]*a_ct + [4]*b_ct + [3 if 3 in pay else 5]):
                            match = ' <<'
                    print(f"  t={t:4d} {ds} h={hd:2d} A={a_ct} B={b_ct} {mk} pay={pay[:10]}")

        # Save
        sol = {}
        for (q, s), (qo, so, do) in table.items():
            sol[f"{q},{s}"] = {"new_state": qo, "new_symbol": so, "direction": do}
        with open("solution_universal.json", "w") as f:
            json.dump(sol, f, indent=2)
        print("\nSaved to solution_universal.json")
        return table
...
if __name__ == "__main__":
    ...
    p.add_argument("--spc-max", type=int, default=24)
